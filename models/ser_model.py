...
from models.multiHeaded_audio_attention import MultiheadAttention
...
class Ser_Model(nn.Module):
    def __init__(self):
        ...
        # WAV2VEC 2.0
        self.wav2vec2_model = Wav2Vec2Model.from_pretrained("/home/puravsingla011/SER_0915/features_extraction/pretrained_model/wav2vec2-base-960h")
        ...
        self.spec_attention = MultiheadAttention(256*36,256*36,12)
        self.mfcc_attention = MultiheadAttention(300*512, 300*512, 12)
        self.audio_wav_attention = MultiheadAttention(149*768, 149*768, 12)
    ...
    def forward(self, audio_spec, audio_mfcc, audio_wav):      
        ...
        # audio_wav: [32, 48000]
            # audio_spec.reshape(1, audio_spec.shape[0], audio_spec.shape[1]*audio_spec.shape[2]*audio_spec.shape[3])
            # audio_mfcc.reshape(1, audio_mfcc.shape[0], audio_mfcc.shape[1]*audio_mfcc.shape[2])
            # audio_wav.reshape(1, audio_wav.shape[0], audio_wav.shape[1])
            # #[1(batch_size_for attention model, batch(seq_length for attention_model), elements(embedding_size for model)]
            # audio_spec = self.spec_attention(audio_spec)
            # audio_mfcc = self.mfcc_attention(audio_mfcc)
            # audio_wav = self.audio_wav_attention(audio_wav)

            # audio_spec.reshape(32, 3, 256, 384)
            # audio_mfcc.reshape(32, 300, 40)
            # audio_wav.reshape(32, 48000)
        ...
        audio_spec = audio_spec.reshape(32, 256, 36)
        ###### ======= ######
        ...
        audio_mfcc.reshape(1, audio_mfcc.shape[0], audio_mfcc.shape[1]*audio_mfcc.shape[2])
        audio_mfcc = self.mfcc_attention(audio_mfcc)
        audio_mfcc = self.reshape(32, 300, 512)
        ###### ======= ######
        ...
        #######   ===== #####
        audio_wav.reshape(1, audio_wav.shape[0], 149*768)
        audio_wav = self.audio_wav_attention(audio_wav)
        audio_wav.reshape(32, 149, 768)
        ###### ======= ######
